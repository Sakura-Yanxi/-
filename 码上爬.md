...
function sakura (p) {
    let page = 'page='+ p.toString()
    console.log(page)
    var _0x350b3a = _0x1e5c29();
    var t = Date["parse"](new Date());
    var _0x350b3a = _0x1e5c29();
    var r = x1["stringify"](_0x321804(_0x10b283(page))).toString();
    console.log(r)
    var s = mmm(r + _0x350b3a +t);
...
```
## 14.神枪收
#### 经过观察请求头里面并没有加密。负载params里面有相应的m值加密。响应中没有相应的加密。按照惯例观察loadPage堆栈发现里面并没有任何加密的痕迹。然后发现侧边栏有相应的xhr.js文件。尝试分析这个js文件，发现有ob混淆，解混淆后观察代码发现有相应的ts生成位置并且下面有return dd尝试在浏览器断点调试此处，发现正式m值的生成位置处因此即可扣js代码。并不全扣，发现解混淆后的代码主要分成四大部分，全扣可能更加复杂，因此只扣x函数的那个全部其余的慢慢补，并尝试删除一部分最终完整代码如下:
```js
...
function Sakura() {
    const b = "/api/problem-detail/14/data/"
    const __sk_GH = {
      "G": 409
    },
          f = {
    ...
    if (z["qlWPI"]("HbEiW", "HbEiW")) {
      ...
      return dd += z["fDawx"](s, function (N) {
        ...
        for (; z["BohQq"](I, N["length"]);) {
          ...
          S = 63 & k;
          isNaN(y) ? K = S = 64 : z["XmzRs"](isNaN, k) && (S = 64);
...
cookies = {
    "sessionid": "28xvqtouyogiovqb2hxbujws2rkkrruz",
    "v": "QTNSMU8zdjhyeVR0SVRRQTNsdkxFV241UlRualRaa2Yybk1zYmc3VkFHT0F4eHBuTmwxb3h5cUJfQTFkMTc2MDI1NjE1ODkyMw==",
    "Hm_lvt_0d2227abf9548feda3b9cb6fddee26c0": "1760194337,1760252172,1760255872,1760274530",
    "HMACCOUNT": "FA452653B0C48E17",
    "Hm_lpvt_0d2227abf9548feda3b9cb6fddee26c0": "1760275617"
}
url = "https://www.mashangpa.com"
list = []
for page in range(1, 21):
    with open('4.js','r',encoding='utf-8') as f:
        ctx = execjs.compile(f.read())
        result = ctx.call('Sakura')
    data = {
        "page": page
    }
    parseurl = url + result
    data = json.dumps(data, separators=(',', ':'))
    response = requests.post(parseurl, headers=headers, cookies=cookies,data=data).json()
    list += response['current_array']
    print(response)
    time.sleep(0.5)
print(sum(list))
